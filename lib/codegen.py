#######################################################
#  This file was initially developed by the Sympy project (sympy.utilities.codegen). 
#  It was slightly modified here to use cse to eliminate common subexpressions 
#  thus making the output Fortran code more readable and comforming to 
#  Fortran standard.
#  The code is released under the BSD license (same as Sympy's license).
...
def update_progress(progress, total):
    ...
    progress=float(round(progress/total,4))
...
#        print(expressions)
        # local variables
        print('exp_len',len(expressions))
        sys.stdout.flush()

        # temp solution to speed things up
        local_vars = set()
        #set([i.label for i in expressions.atoms(Idx)])
        print('local_len',len(local_vars))
        sys.stdout.flush()
#        print(local_vars)
        # global variables
        ...
        sys.stdout.flush()
        self.subexpr, self.simplified_expr = cse(expr, order='none')
        print('CSE')
        sys.stdout.flush()
...
class FCodeGen(CodeGen):
    ...
    def _call_printer(self, routine):
        ...
        print(len(self.subexpr))
        num=0
        start = time.time()
        total=len(self.subexpr)
        for var, exp in self.subexpr:
            num+=1
            code_list.append("REAL*8 :: %s\n" % (
                    var))
        ...
        for var, exp in self.subexpr:
            num+=1
            f_expr=(fcode(exp, assign_to=var, source_format='free', human=False)[2])
    #        print(f_expr)
            code_list.append("%s\n" %(f_expr))
            update_progress(num, total)


        tmp=self.simplified_expr
        print(len(tmp))
        sys.stdout.flush()
        
        num=0
        total=len(tmp)
        for ni in xrange(len(tmp)):
            num+=1
            update_progress(num, total)
            result=routine.result_variables[ni]
            if isinstance(result, Result):
            ...
            exp=tmp[ni]
            constants, not_fortran, f_expr=(fcode(exp, assign_to=assign_to, source_format='free', human=False))
            print(constants)
            sys.stdout.flush()
            print(not_fortran)
            sys.stdout.flush()
